-- =============================================================
-- v10-submit-approval-add-ptw.sql
-- =============================================================
-- Extends public.submit_approval(text, uuid, ...) to accept
-- doc_type='ptw'. The v9 function shipped with the doc_type
-- guard restricted to ('si','vo') even though approvals.doc_type
-- already accepts 'ptw' and v10-split/3-trg-approval-ptw-branch.sql
-- added the chain-advance + dispatch branch for it.
--
-- Without this fix every PTW approval submitted from
-- PtwApproverBar fails with `unsupported doc_type ptw` and the
-- PTW chain (safety_officer -> main_contractor) never advances to
-- 'active' — blocking @ptw-smoke, @ptw-fire-watch-smoke and the
-- whole Phase 3 PTW happy path.
--
-- NON-DESTRUCTIVE: replaces the function body only. IDEMPOTENT:
-- create or replace.
-- =============================================================
